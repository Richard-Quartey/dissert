...
a. XAMPP Control panel - This application was used to connect phpMySQL to the web application as the
backend database.
*********Install XAMPP*******
...
3. Open XAMPP Control Panel and start the Apache and MySQL modules. If MySQL module is not running on
...
5. When in phpMySQL first create a new database name [nodejs-login] and then from the IMPORT tab, import SQL file
 from application folder with name: [database -> users.sql] . Database details as follows [host: 'localhost', user: 'root', password: '']
...
2. Run Hyper
3. Download and extract the web application zip file from Github: [https://github.com/Richard-Quartey/dissert] and in Hyper navigate
  to downloaded project folder [dissert-master]
...
6. Start web application on localhost in web browser [localhost:3000]
